...
WITH aggTable AS (
	SELECT
		...
		CASE WHEN category.name IN ('Animation', 'Children', 'Classics', 'Comedy', 'Family', 'Music') THEN 'Kid Friendly' ELSE 'No Kids' END AS friendly,
...
SELECT DISTINCT(aggTable.category),
	aggTable.friendly,
	aggTable.quartile,
...
FROM
	ranking_table
JOIN
	payment ON payment.customer_id = ranking_table.cust_id
WHERE
	ranking_table.pay_rank BETWEEN 1 and 10
GROUP BY
	customer_name,
	monthYear,
	ranking
...
SELECT
	...
	CASE WHEN LAG(by_month.customer_name) OVER (ORDER BY by_month.ranking, by_month.monthYear)=by_month.customer_name THEN LAG(by_month.payment) OVER (ORDER BY by_month.ranking, by_month.monthYear)
			 ELSE null
	END AS prior_payment,
	/*Only calculate differnce when it's the same person in both months*/
	CASE WHEN LAG(by_month.customer_name) OVER (ORDER BY by_month.ranking, by_month.monthYear)=by_month.customer_name THEN by_month.payment - LAG(by_month.payment) OVER (ORDER BY by_month.ranking, by_month.monthYear)
			 ELSE null
	END AS difference
FROM
	by_month;
